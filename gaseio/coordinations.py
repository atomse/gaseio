...
coordinates part in quantum chemistry softwares
...
def input_standard_pos_transform(inp_pos, std_pos, t_vals,
    ...
    R_mat = get_std_inp_R_mat(inp_pos, std_pos)
...
def get_std_inp_R_mat(inp_pos, std_pos):
    """
    R_mat: rotate standard positions --> input positions
           std_m * R_mat = inp_m
    """
    std_O = np.array(std_pos)[-1].copy()
    inp_O = np.array(inp_pos)[-1].copy()
